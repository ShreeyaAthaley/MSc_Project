...
m.K = pyo.Set(initialize=range(10)) 
m.KI = pyo.Set(within=m.K, initialize=[1,2,3,4,5,6,7,8,9]) #Intervals: KI=K/{0}
#%%
#cost
...
m.TIN_H = pyo.Var(m.I, bounds=fb1, initialize={1: 413.7573, 2: 393.2211, 3: 393.2211, 4:569.9799, 5: 343.7344, 6:343.7344, 7: 523.15})
m.TIN_C = pyo.Var(m.J, bounds=fb2, initialize={1: 333.6259, 2: 298.8512, 3: 383.1126, 4:293.15, 5:248.15})
m.TOUT_H = pyo.Var(m.I, bounds=fb3, initialize={1: 294.4628, 2: 289.9245, 3:314.0815, 4: 301.3900, 5: 311.2158, 6:342.7344, 7: 522.15})
m.TOUT_C = pyo.Var(m.J, bounds=fb4, initialize={1: 512.4354, 2: 359.2733, 3: 384.1126, 4:298.15, 5:249.15})

#temperature on interval boundaries
m.T_k= pyo.Var(m.K, bounds=[0,1000],initialize={0:550,1:500,2:450,3:400,4:350,5:300,6:250,7:200,8:190,9:150})

#%%
m.THIN_link1=pyo.Constraint(expr=m.TIN_H[1]==140.6073+273.15)
m.THIN_link2=pyo.Constraint(expr=m.TIN_H[2]==120.0711+273.15)
m.THIN_link3=pyo.Constraint(expr=m.TIN_H[3]==120.0711+273.15)
m.THIN_link4=pyo.Constraint(expr=m.TIN_H[4]==296.8299+273.15)
m.THIN_link5=pyo.Constraint(expr=m.TIN_H[5]==70.5844+273.15)
m.THIN_link6=pyo.Constraint(expr=m.TIN_H[6]==70.5844+273.15)

m.THOUT_link1=pyo.Constraint(expr=m.TOUT_H[1]==21.3128+273.15)
m.THOUT_link2=pyo.Constraint(expr=m.TOUT_H[2]==16.7745+273.15)
m.THOUT_link3=pyo.Constraint(expr=m.TOUT_H[3]==40.9315+273.15)
m.THOUT_link4=pyo.Constraint(expr=m.TOUT_H[4]==28.2400+273.15)
m.THOUT_link5=pyo.Constraint(expr=m.TOUT_H[5]==38.0658+273.15)
m.THOUT_link6=pyo.Constraint(expr=m.TOUT_H[6]==70.5844+273.15-1)

m.TCIN_link1=pyo.Constraint(expr=m.TIN_C[1]==60.4759+273.15)
m.TCIN_link2=pyo.Constraint(expr=m.TIN_C[2]==25.7012+273.15)
m.TCIN_link3=pyo.Constraint(expr=m.TIN_C[3]==109.9626+273.15)

m.TCOUT_link1=pyo.Constraint(expr=m.TOUT_C[1]==239.2854+273.15)
m.TCOUT_link2=pyo.Constraint(expr=m.TOUT_C[2]==86.1233+273.15)
m.TCOUT_link3=pyo.Constraint(expr=m.TOUT_C[3]==109.9626+273.15+1)
#%%
...
m.F_H = pyo.Var(m.IP, bounds=fb5, initialize={1: 22.2722, 2: 22.6167, 3:25.4667, 4:540.0105 , 5: 615.1357, 6: 21971.7512, 7:10000})
m.F_C = pyo.Var(m.J, bounds=fb6, initialize={1: 537.5155, 2:499.7202 ,3:16162.7088, 4:5000, 5: 5000})
#%%
m.FH_link1=pyo.Constraint(expr=m.F_H[1]==2000/3600*40.09)
m.FH_link2=pyo.Constraint(expr=m.F_H[2]==2000/3600*40.71)
m.FH_link3=pyo.Constraint(expr=m.F_H[3]==2000/3600*45.84)
m.FH_link4=pyo.Constraint(expr=m.F_H[4]==57405.5182/3600*33.865)
m.FH_link5=pyo.Constraint(expr=m.F_H[5]==1949.5231/3600*1135.913)
m.FH_link6=pyo.Constraint(expr=m.F_H[6]==21971.7512)#directly equate to predicted duty 

m.FC_link1=pyo.Constraint(expr=m.F_C[1]==60898.6931/3600*31.775)
m.FC_link2=pyo.Constraint(expr=m.F_C[2]==3844.1411/3600*467.983) 
m.FC_link3=pyo.Constraint(expr=m.F_C[3]==16162.7088)#directly equate to predicted duty 
#%%
# Duty of H
def QH_rule1(m, i, k):
...
#'limits/time': 60,
solver.options ={'limits/time': 60, 'limits/gap': 0.01, 'numerics/feastol': 1e-4, 'numerics/dualfeastol': 1e-4, 'propagating/obbt/dualfeastol': 1e-04}#,'heuristics/multistart/maxboundsize': 10001}
